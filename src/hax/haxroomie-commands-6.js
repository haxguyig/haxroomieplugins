...
room.onPlayerChat = function (player, message) {
    ...
    if (["afk"].includes(split_message)){
        ...
        colors_dict = {0: 0xFFFFFF, 1: xe86c54, 2: 0x588ce4}
        ...
        try{
            person = adminLogin[split_message_old[1]]
            room.setPlayerAdmin(player.id, true)
            room.sendChat(person+" has logged in!")
            return false
        }
        catch(err) {
            return false
...
room.onGameTick = function(){
    ...
    if (Number.isInteger(ticks/18000)){
        room.sendAnnouncement("Come join the SBNL Discord! https://discord.gg/uqfusuxrDX", null, 0x1e81b0, "normal", 1)
    }
}
