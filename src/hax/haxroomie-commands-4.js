...
var paused = false;
var ticks = 0;
var adminLogin = {"richana": "richy", "nfana": "Nfab", "namana": "Name"}
...
room.onPlayerJoin = function(player) {
    room.sendChat("Welcome "+player.name+" to the HSN room!");
    updateAdmins();
...
room.onPlayerKicked = function(player, reason, ban, byPlayer){
    if (bannedAdmin){
        ...
        else bannedAdmin = false
    }
    if (ban) action = "BANNED"
    else action = "KICKED"
    sendAnnouncementToDiscord("__**PLAYER "+action+"!**__\n**NAME:** "+player.name+" **ID:** "+player.id.toString()+"\n**BY PLAYER:** "+byPlayer.name+" **ID:** "+byPlayer.id+"\n**REASON:**"+reason);
    sendAnnouncementToDiscord("_ _")
}
...
function sendAnnouncementToDiscord(message) {
    ...
    request.open("POST", "https://discord.com/api/webhooks/871270071987765298/Rndt5Y2CA4IcOSa1uZLmuI-seMWF5dksPoFdTB6YxCA9oDBnnRH3EtlpB_7G7ji6ZOj5");
...
room.onPlayerChat = function (player, message) {
    ...
    sendAnnouncementToDiscord("**NAME:** "+player.name+" **ID:** "+player.id.toString()+"\n**MESSAGE:** "+message);
    sendAnnouncementToDiscord("_ _")
    ...
    if (["afk"].includes(split_message)){
        ...
        room.sendChat("NFLCHL Discord Link: https://discord.gg/ZfX9XXtb45")
    }
        ...
        command_desc =
        {
            ...
            "discord": "Invites you to the NFLCHL discord! https://discord.gg/ZfX9XXtb45",
        ...
        if (command_desc[split_message_old[1]]){
            room.sendAnnouncement("(DM) "+ command_desc[split_message_old[1]], player.id, 0x1e81b0)
        }
        else {
    ...
    if (player.admin)
    {
        ...
        else if (["auto"].includes(split_message)){
            autoRand("auto");
        }
        else if (["rand", "random"].includes(split_message)){
            autoRand("rand");
        }
...
room.onGameTick = function(){
    ...
    if (Number.isInteger(ticks/18000)){
        room.sendAnnouncement("Come join the NFLCHL Discord! https://discord.gg/ZfX9XXtb45", null, 0x1e81b0, "normal", 1)
    }
}
